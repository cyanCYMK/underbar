...
(function() {
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(allTrue, item){
      ...
      return args.length === 1 ? Boolean(item) : Boolean(iterator(item));
    }, true);
  };
  ...
  _.some = function(collection, iterator) {
    ...
    if (arguments.length === 1) {var iterator = _.identity;}
  ...
  _.defaults = function(obj) {
    ...
    for (var i=1, l=arguments.length; i<l; i++){
      _.each(arguments[i], function(item, key, collection){
        if (originalObj[key] == undefined) {originalObj[key] = item;}
      });
    }
    return originalObj;
  };
  ...
  _.memoize = function(func) {
    ...
    return function(){
      var arg = arguments[0];
      if (result[arg] === undefined) {result[arg] = func.apply(this, arguments);}
      return result[arg];
    };
  };
  ...
  _.shuffle = function(array) {
    var copiedArray = array.slice(0), randomArray = [];
    var newIndex;
    for (var i=0, l=copiedArray.length; i<l; i++){
      newIndex = Math.floor(Math.random() * copiedArray.length);
      randomArray.push(copiedArray.splice(newIndex, 1)[0]);
    }
    return randomArray;
  };
  ...
  _.sortBy = function(collection, iterator) {
    ...
    _.each(collection, function(value, index){
      if (typeof iterator === 'string'){
        valuesToSortBy.push([index, value[iterator]]);
      } else if (typeof iterator === 'function'){
        valuesToSortBy.push([index, iterator(value)]);
      }
    });
    valuesToSortBy.sort(function(a,b){
      if(a[1] > b[1] || typeof a[1] === 'undefined'){
        return 1;
      } else if(a[1] < b[1]){
        return -1;
      } else {
    ...
    var sortedCollection = [], index;
    for (var i=0; i<collection.length; i++){
      index = valuesToSortBy[i][0];
      sortedCollection.push(collection[index]);
    }
  ...
  _.zip = function() {
    ...
    for (var i=0, l=arguments.length; i<l; i++){
      if (arguments[i].length > biggest.length) {biggest = arguments[i];}
    }
    var result = [], inner = [];
    for (var j=0; j<biggest.length; j++){
      for (var k=0; k<arguments.length; k++){
        inner.push(arguments[k][j]);
      }
      result.push(inner);
      inner = [];
    }
  ...
  _.flatten = function(nestedArray, result) {
    ...
    loopThruArray(nestedArray, result);
    return result;
    function addIfNotArray(value, result){
      if (!Array.isArray(value)){
        result.push(value);
      }
      return result;
    }
    function loopThruArray(array, result){
      _.each(array, function(value, index, array){
        result = addIfNotArray(value, result);
        if (Array.isArray(value)){
          loopThruArray(value, result);
        }
      });
    }
  };
